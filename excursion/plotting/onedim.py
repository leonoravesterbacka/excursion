...
def plot_gpytorch(axarr, gps, likelihood, X, y_list, scandetails, batchsize=1):
    ...
    for ax in gp_axes:
        ...
        if ax != axarr[-1]:
            ...
            observed_pred = likelihood(
                gps(torch.tensor(scandetails.plotX, dtype=torch.float64))
            )
            ...
            ax.plot(
                scandetails.plotX[:, 0], observed_pred.mean.detach().numpy()
            )  # scandetails.plotX[:,0] is a numpy array
            mu_stds.append(observed_pred)

            # plot confidence region
            ax.fill_between(
                scandetails.plotX[:, 0], lower.detach().numpy(), upper.detach().numpy()
            )
            ...
            for thr in scandetails.thresholds:
                ax.hlines(
                    thr,
                    np.min(scandetails.plotX),
                    np.max(scandetails.plotX),
                    colors="grey",
                )
            ax.set_xlim(np.min(scandetails.plotX), np.max(scandetails.plotX))
            ax.set_ylim(*scandetails.y_lim)
    ...
    entraxis.fill_between(
        scandetails.plotX[:, 0], -entropies, entropies, color="steelblue"
    )
    entraxis.set_ylim(-1, 1)
    entraxis.axhline(0, c="k")
...
def plot_GP_dual(gp1, gp2, testcase, **kwargs):
    ...
    plt.subplot(1, 2, 1)
    plot_GP(
        gp1, testcase, acq=acq1, acq_type="PES", x_new=xnew1, ax0=axes[0], ax1=axes[2]
    )
    plt.subplot(1, 2, 2)
    plot_GP(
        gp2, testcase, acq=acq2, acq_type="MES", x_new=xnew2, ax0=axes[1], ax1=axes[3]
    )
    plt.tight_layout()
